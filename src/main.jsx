...
import { parseRoute, parseNavTarget, hrefFor, hrefFromTarget, hashFor, linkClick, findCar, carRef, writeLocation } from './routing.js';
import './currency.css';
import './portal.css';
...
function ExtraPages2({type,navigate,openAuction}){
 ...
 {type==='brands'&&<><div className="brand-grid">{BRANDS().map(b=><article className="brand-card" key={b.name}><span className="brand-tile"><img loading="lazy" decoding="async" width="56" height="34" src={LOGO(b.name)} alt={b.name+" logo"}/><small>{b.name}</small></span><div className="brand-body"><div><b>{b.count} vehicles</b><span>now in stock</span></div><div className="brand-models">{b.models.map(m=><a href="/inventory" onClick={linkClick('inventory',()=>{pendingMake=b.name;navigate('inventory')})} key={m}>{m}</a>)}</div><PageLink className="outline-btn" to="inventory" navigate={()=>{pendingMake=b.name;navigate('inventory')}}>View {b.name} stock <ArrowRight/></PageLink></div></article>)}</div><div className="demo-strip">{BRANDS().length===0?null:[['12','Brands catalogued'],['30','Vehicles in stock'],['100%','Auction-sourced'],['24h','New stock update']].map(x=><div key={x[0]}><b>{x[0]}</b><span>{x[1]}</span></div>)}</div></>}
...
function App(){
 ...
 return <div className="site">
  <div className="grain"/><header className="nav-wrap"><nav className="nav shell">
   ...
   <div className={'navlinks '+(menu?'open':'')}>
    <div className="nav-more">
     <button className="more-btn">Inventory <ChevronDown className="more-chev"/></button>
     <div className="more-panel">
      {[
        [Layers,'Japan dealer stock','japan-stock'],[CarFront,'Brands','brands']
    ...
    <div className="nav-more">
     <button className="more-btn">More <ChevronDown className="more-chev"/></button>
     <div className="more-panel">
      {[
        [Globe2,'World network','world'],[CarFront,'Inventory','inventory'],[Gavel,'Live auctions','auction'],[Wrench,'Services','services'],[Ship,'Shipping','shipping'],[MapPin,'Destinations','destinations'],[BookOpen,'How to buy','howbuy'],[Calculator,'Calculators','tools'],[Newspaper,'News & guides','news'],[MessageCircle,'Reviews','reviews'],[ClipboardCheck,'Help & FAQ','faq'],[BadgeCheck,'About AR7','about'],[Landmark,'Staff CRM','crm'],[Mail,'Contact us','contact']
      ].map(x=>{const I=x[0];return <a key={x[2]} href={hrefFor(x[2])} onClick={linkClick(x[2],navigate)}><i><I/></i><span>{x[1]}</span></a>})}
     </div>
    </div>
  ...
  <main>
   ...
   </>:page==='world'?<WorldPage navigate={navigate}/>:page==='account'?<CustomerAccountPage navigate={navigate}/>:page==='studio'?<DeviceStudio navigate={navigate}/>:<InnerPage page={page} navigate={navigate} vehicleId={vehicleId} openAuction={()=>setModal(true)} favs={favs} setFavs={setFavs}/>}
  </main>
