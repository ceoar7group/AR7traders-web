...
cars.forEach((c,i)=>{c.doors=DOORS[c.body]||4;c.chassis=(CHASSIS[c.make]||'6AA-0000')+'-'+(1200+i*7);c.int=c.int||['Gray','Black','Beige'][i%3];c.ven=['USS Tokyo','TAA Kinki','JU Aichi','CAA Chubu','HAA Kobe'][i%5];c.arr=new Date(2026,7,1+((i*3)%28)).toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'});c.feats=FEATPOOL.filter((f,k)=>((i*31+k*13)%11)<6).slice(0,6).concat(['New battery','Service records'])});
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
const enrichCar=(c,i)=>{c.doors=DOORS[c.body]||4;c.chassis=(CHASSIS[c.make]||'6AA-0000')+'-'+(1200+i*7);c.int=c.int||['Gray','Black','Beige'][i%3];c.ven=c.ven||['Goo-net dealer stock','Goo-net dealer stock','Goo-net dealer stock','Goo-net dealer stock','Goo-net dealer stock'][i%5];c.arr=c.arr||new Date(2026,7,1+((i*3)%28)).toLocaleDateString('en-GB',{day:'2-digit',month:'short',year:'numeric'});c.feats=c.feats||FEATPOOL.filter((f,k)=>((i*31+k*13)%11)<6).slice(0,6).concat(['New battery','Service records']);return c};
...
const galleryFor=c=>{
 ...
 return c.image?[c.image]:['/assets/ar7-mark.png'];
};
...
function HeroVisual({navigate}){
 ...
 return <div className="hero-visual" ref={wrap} onMouseMove={onMove} onMouseLeave={onLeave}>
  ...
  <div className="hero-card car-main" onClick={()=>navigate('inventory')} title="View rotating stock">
  ...
  <div className="floating-badge"><BadgeCheck/><span>Verified<br/><b>Goo-net stock</b></span></div>
 </div>}
...
function ExtraPages2({type,navigate,openAuction}){
 ...
 {type==='brands'&&<><div className="brand-grid">{BRANDS().map(b=><article className="brand-card" key={b.name}><span className="brand-tile"><img loading="lazy" decoding="async" width="56" height="34" src={LOGO(b.name)} alt={b.name+" logo"}/><small>{b.name}</small></span><div className="brand-body"><div><b>{b.count} vehicles</b><span>in stock</span></div><div className="brand-models">{b.models.map(m=><button onClick={()=>{pendingMake=b.name;navigate('inventory')}} key={m}>{m}</button>)}</div><button className="outline-btn" onClick={()=>{pendingMake=b.name;navigate('inventory')}}>View {b.name} stock <ArrowRight/></button></div></article>)}</div><div className="demo-strip">{BRANDS().length===0?null:[['12','Brands catalogued'],['41','Stocked vehicles'],['100%','Goo-net verified'],['24h','New stock update']].map(x=><div key={x[0]}><b>{x[0]}</b><span>{x[1]}</span></div>)}</div></>}
  ...
  <footer className="site-footer">
   ...
   <div className="shell footer-grid">
    <div className="footer-brand">
     ...
     <div className="socials"><button onClick={()=>navigate('inventory')} title="Vehicle gallery"><Camera/></button><button onClick={()=>navigate('reviews')} title="Customer stories"><MessageCircle/></button><button onClick={()=>navigate('contact')} title="Contact AR7"><Send/></button><a className="wa-link" href={waLink(settings.whatsapp_number,settings.whatsapp_message)} target="_blank" rel="noopener noreferrer" title="WhatsApp AR7"><WhatsAppIcon size={15}/></a></div>
    </div>
    <div><b>EXPLORE</b><a onClick={()=>navigate('inventory')}>Inventory<ArrowUpRight/></a><a onClick={()=>navigate('auction')}>Auction access<ArrowUpRight/></a><a onClick={()=>navigate('services')}>Services<ArrowUpRight/></a><a onClick={()=>navigate('brands')}>Brands<ArrowUpRight/></a><a onClick={()=>navigate('destinations')}>Destinations<ArrowUpRight/></a><a onClick={()=>navigate('tools')}>Calculators<ArrowUpRight/></a><a onClick={()=>navigate('world')}>World network<ArrowUpRight/></a></div>
    <div><b>COMPANY</b><a onClick={()=>navigate('howbuy')}>How to buy<ArrowUpRight/></a><a onClick={()=>navigate('news')}>News & guides<ArrowUpRight/></a><a onClick={()=>navigate('about')}>About us<ArrowUpRight/></a><a onClick={()=>navigate('reviews')}>Customer stories<ArrowUpRight/></a><a onClick={()=>navigate('faq')}>Help & FAQ<ArrowUpRight/></a>{signedIn&&<a onClick={()=>navigate('account')}>My account<ArrowUpRight/></a>}<a onClick={()=>navigate('portal')}>Portal tour<ArrowUpRight/></a><a onClick={()=>navigate('crm')}>Staff CRM<ArrowUpRight/></a>{!signedIn&&<a onClick={()=>navigate('account')}>Customer sign up<ArrowUpRight/></a>}</div>
    <div><b>GET IN TOUCH</b><a href={'mailto:'+settings.contact_email}><Mail/> {settings.contact_email}</a><a href={telHref(settings.contact_phone)}><Phone/> {settings.contact_phone}</a><a className="wa-link" href={waLink(settings.whatsapp_number,settings.whatsapp_message)} target="_blank" rel="noopener noreferrer"><WhatsAppIcon size={15}/> WhatsApp</a><a onClick={()=>navigate('contact')}><MapPin/> {settings.contact_address}</a>
   ...
   <div className="shell footer-bottom"><span>© 2026 AR7 Traders. All rights reserved.</span><span><a onClick={()=>navigate('faq')}>Privacy</a> · <a onClick={()=>navigate('faq')}>Terms</a> · <a onClick={()=>navigate('faq')}>Export policy</a></span><div className="footer-badges"><span><BadgeCheck/> Certified stock</span><span><LockKeyhole/> Secure payments</span></div><b>AR7TRADERS.COM</b></div>
  </footer>
