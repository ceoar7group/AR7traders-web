...
const priceOf=c=>Number(c.price.replace(/[$,]/g,''))||0;
...
const useCarPrice=()=>{const {fmt,isBase}=useCurrency();return c=>isBase?(c.price||'—'):fmt(priceOf(c))};
...
function VehicleCard({c,onOpen,comp,onCmp}){const price=useCarPrice();return <article className="car-card page-car" onClick={()=>onOpen?.(c)}>
  ...
  <main>
   ...
   <section className="inventory shell section" id="inventory">
    ...
    <div className="car-grid">{shown.map((c,i)=><article className="car-card" key={c.id} style={{'--delay':i*80+'ms'}}>
      ...
      <div className="car-info"><div className="make"><img loading="lazy" decoding="async" width="34" height="22" src={LOGO(c.make)} alt=""/>{c.make}</div><h3>{c.model}</h3><div className="specs"><span><CalendarDays/> {c.year}</span><span><Gauge/> {c.km} km</span><span><Fuel/> {c.fuel}</span><span><ArrowLeftRight/> {c.tr}</span></div><div className="car-bottom"><div><small>EXPORT PRICE FROM</small><b>{price(c)}</b></div><button onClick={()=>navigate('inventory')} aria-label="View vehicle"><ArrowUpRight/></button></div><div className="loc"><MapPin/> {c.location}, Japan</div></div>
    </article>)}</div>
